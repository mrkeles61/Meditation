...
### 1.5 Reranking Retrieved Documents
...
def llm_rerank(query: str, docs: list, top_k: int = 5) -> list:
    """Use LLM to rerank when semantic understanding needed"""
    prompt = f"""Given this query: "{query}"
    
Rank these documents by relevance (most relevant first):
{chr(10).join([f'{i+1}. {doc.content[:200]}...' for i, doc in enumerate(docs)])}
...
### 1.6 Hierarchical Retrieval

```python
class HierarchicalRetriever:
    """
    Multi-level retrieval for better precision:
    1. First pass: coarse retrieval (document/section level)
    2. Second pass: fine-grained retrieval (paragraph level)
    """
    
    def __init__(self, coarse_index, fine_index):
        self.coarse_index = coarse_index  # Document-level embeddings
        self.fine_index = fine_index      # Chunk-level embeddings
    
    def retrieve(self, query: str, top_k: int = 5) -> list:
        # Stage 1: Get relevant documents
        doc_ids = self.coarse_index.search(query, top_k=20)
        
        # Stage 2: Search within those documents only
        results = self.fine_index.search(
            query, 
            top_k=top_k,
...
### 1.7 RAG Anti-Patterns & Sharp Edges

```python
# ❌ ANTI-PATTERNS TO AVOID

# 1. Fixed-size chunking breaks context
# BAD:
chunks = text.split_every(512)  # Breaks mid-sentence

# GOOD: Use semantic boundaries
chunks = recursive_split(text, separators=["\n\n", "\n", ". "])

# 2. No overlap loses context
# BAD:
ChunkConfig(size=512, overlap=0)

# GOOD: 10-20% overlap
ChunkConfig(size=512, overlap=50)

# 3. Same embedding model for different content types
# BAD: Using text embeddings for code
embed("def hello(): print('world')")

# GOOD: Evaluate per content type
code_embedder = load_model("code-specific-model")
text_embedder = load_model("text-embedding-3-small")

# 4. Not refreshing embeddings when docs change
# GOOD: Track document versions
def sync_embeddings(doc_id: str, new_content: str, old_hash: str):
    new_hash = hash(new_content)
    if new_hash != old_hash:
        vector_db.delete(doc_id)
        vector_db.upsert(doc_id, embed(new_content))
```

| Issue | Severity | Solution |
|-------|----------|----------|
| Fixed-size chunks break sentences | Critical | Use recursive splitter with semantic boundaries |
| Query/doc embeddings from different models | Critical | Ensure consistent embedding model |
| RAG adds latency | High | Add caching, use smaller models for retrieval |
| Stale embeddings after doc updates | Medium | Implement embedding refresh pipeline |
| Pure semantic without keyword fallback | Medium | Use hybrid search with BM25 |
